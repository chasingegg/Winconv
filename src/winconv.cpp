...
void winconv_init_lib()
{
    ...
    t_filter = (float *)mkl_malloc(16*FSTRIDE*sizeof(float), 64);
    assert(t_filter != NULL); 
    t_image = (float *)mkl_malloc(16*ISTRIDE*sizeof(float), 64);
    assert(t_image != NULL); 
    c_out = (float *)mkl_malloc(16*OSTRIDE*sizeof(float), 64);
    assert(c_out != NULL); 
}
		...
		int *bblock2x3)
{
    ...
    /* F(2,3) */
    for(int i = 0; i < lyn; i++){
	m_used  = 1.0f*(N*(H[i]-2)/2*(W[i]-2)/2*C[i] + C[i]*K[i] + N*(H[i]-2)/2*(W[i]-2)/2*K[i])/1024/1024/1024*16;
